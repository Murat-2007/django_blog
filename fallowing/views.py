...
def kullanici_modal_takip_et_cikar(request):
    ...
    #kendi profilimmis gibi, takipcilerimi tekrar cekiyorum
    ...
    if owner ==request.user.username:
        ...
        html_render_takip_durum = render_to_string('auths/profile/include/fallowing/fallowing_partion.html', context=context, request=request)
        
        
        if fallow_type == "fallowed":
            fallowing = Fallowing.get_fallowed(user=request.user)
            falloving=fallowers_and_fallowed_paginate(fallowing, 1)
            html = render_to_string('fallowing/profile/include/fallowing_fallowed_list.html',
                context={'fallowing':fallowing, 'my_fallowed':my_fallowed, 'fallow_type': fallow_type},request=request)
            
            html_paginate = render_to_string('fallowing/profile/include/button_include/show_more_button.html',
                context={'username': request.user.username, 'fallowing': fallowing, 'fallow_type': fallow_type})
    ...
    #yani bu profil baskasina aitse
...
def kullanici_takip_et_cikar_for_post(request):
    ...
    html = render_to_string('blog/include/favorite/favorite_user_obj.html', context={'user': takip_edilen_kullanici, 'my_fallowed_user': my_fallowed_user}, request=request)
...
def sub_kullanici_takip_et_cikar(request):
    ...
    if not takip_ediyor_mu:
        ...
        Fallowing.kullaniciyi_takipten_cikar(fallowed=fallowed, fallower=fallower)
...
def fallowed_or_fallowers_list(request, fallow_type):
    ...
    if fallow_type == 'fallowed':
        ...
        html = render_to_string('fallowing/profile/include/fallowing_fallowed_list.html',
            context={'fallowing': takip_edilenler, 'my_fallowed':my_fallowed, 'fallow_type': fallow_type}, request=request)
        html_paginate = render_to_string('fallowing/profile/include/button_include/show_more_button.html',
            context={'username': user.username, 'fallowing': takip_edilenler, 'fallow_type': fallow_type})
        ...
        print(takipciler)
        html = render_to_string('fallowing/profile/include/fallowing_fallowed_list.html',
            context={'fallowing': takipciler, 'my_fallowed':my_fallowed, 'fallow_type': fallow_type}, request=request)
            
        html_paginate = render_to_string('fallowing/profile/include/button_include/show_more_button.html',
            context={'username': user.username, 'fallowing': takipciler, 'fallow_type': fallow_type})
        #kullaniciyi takip eden kisiler
